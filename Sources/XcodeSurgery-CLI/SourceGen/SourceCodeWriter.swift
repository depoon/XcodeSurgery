...
class SourceCodeWriter {
    ...
    func generateExpressStructExtension(structName: String) -> String {
        return """
            extension \(structName) {
                static func loadPlistIfNeeded() {
                    let variantEncryption = VariantEncryption()
                    let encryptionKeyFilePath = Bundle.main.path(forResource: "encryption-key",
                                                          ofType: nil)
                    let ivFilePath = Bundle.main.path(forResource: "iv",
                                                          ofType: nil)
                    let cipherTextFilePath = Bundle.main.path(forResource: "encryptedPlist",
                                                          ofType: "nil")
                    let decryptionInput = try? variantEncryption.createDecryptionInput(encryptionKeyFilePath:   encryptionKeyFilePath!,
                                             ivFilePath: ivFilePath!, cipherTextFilePath: cipherTextFilePath!)
