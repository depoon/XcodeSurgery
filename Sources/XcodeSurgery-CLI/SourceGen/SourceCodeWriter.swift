...
class SourceCodeWriter {
    ...
    func generatePlistReaderSourceCode(createMode: CreateMode = .normal,
                                       plistFile: String,
                                       structName: String,
        ...
        sourceCode.append(contentsOf: "\nimport XcodeSurgeryKit")
        sourceCode.append(contentsOf: "\n\n//This file is autogenerated from XCScheme")
        sourceCode.append(contentsOf: "\(structCode)")
        
        if createMode == .express, let password = password {
            let expressExtensionCode = self.generateExpressStructExtension(structName: structName,
                                                                           password: password)
            sourceCode.append(contentsOf: "\n\n\(expressExtensionCode)")
        }
    ...
    func generateExpressStructExtension(structName: String, password: Password) -> String {
        
        return """
            extension \(structName) {
                static func loadPlistIfNeeded() {
                    let variantEncryption = VariantEncryption()

                    let password = Password(password: \(structName).encryptionPassword,
                                            salt: \(structName).encryptionPasswordSalt)
                    let ivFilePath = Bundle.main.path(forResource: "iv",
                                                          ofType: nil)
                    let cipherTextFilePath = Bundle.main.path(forResource: "encryptedPlist",
                                                          ofType: nil)
                    let decryptionInput =
                        try? variantEncryption
                                .createDecryptionInput(password:password,
                                                       ivFilePath: ivFilePath!,
                                                       cipherTextFilePath: cipherTextFilePath!)

                    let reader = VariantPlistReader.shared
                    reader.decryptionInput = decryptionInput
                    reader.loadPlistIfNeeded()
                }

                private static let encryptionPassword: String = "\(password.password)"

                private static let encryptionPasswordSalt: String = "\(password.salt)"
            }
        """
    }
}
